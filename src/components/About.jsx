...
import { LuGoal, LuHeartHandshake } from "react-icons/lu";
...
const About = () => {
  ...
  // Vocabulary boxes data
  const vocabItems = [
    { icon: TbSettingsCode, text: "Interactive Exercises", color: "teal" },
    { icon: GiTrophyCup, text: "Daily Word Challenges", color: "orange" },
    { icon: GiBrain, text: "Contextual Learning", color: "purple" },
    { icon: BsStopwatchFill, text: "Spaced Repetition", color: "rose" },
    { icon: FaPenNib, text: "Smart Word Lists", color: "pink" },
    { icon: IoGameController, text: "Gamification", color: "blue" },
    { icon: TbWorld, text: "Immersive Practice", color: "yellow" },
    { icon: GiProgression, text: "Progress Tracking", color: "green" },
  ];
      ...
      <section className=" max-w-6xl mx-auto mb-24">
        <div className="flex flex-col items-center">
          ...
          <p className="animate__animated animate__fadeIn animate__slow animate__delay-1s text-gray-600 text-center max-w-xl mb-6">
            Explore interactive exercises, daily challenges, and gamified learning to expand your vocabulary quickly and enjoyably.
          </p>
        </div>
        ...
        <div className="grid md:grid-cols-3 lg:grid-cols-4 gap-4 ">
          {vocabItems.map((item, idx) => (
            <div
              key={idx}
              className={`animate__animated animate__fadeInUp  bg-${item.color}-100 px-5 py-6 rounded-lg font-semibold flex items-center gap-4 transition-transform duration-300 hover:-translate-y-2 hover:shadow-lg shadow-md`}
            >
              <item.icon className={`text-3xl text-${item.color}-500`} />
              {item.text}
            </div>
          ))}
        </div>
